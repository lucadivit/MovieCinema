<div id="boxext"><fieldset>
    <div id="dati_sala">
        ...
        <label>Seleziona Sala: </label><select name="Sala" id="sala"> <optgroup label="Sala: "></optgroup>{for $i=1 to $numeroSale}<option value="{$i}">{$i}</option>{/for}</select>{if $dati_errati.sala=='TRUE'}{$error_msg.sala}{/if}<br>
